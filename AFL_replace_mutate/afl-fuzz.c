...
string g_prefix_string = "DROP SCHEMA public CASCADE; CREATE SCHEMA public;";
...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           shuffle_queue,             /* Shuffle input queuegg?             */
...
EXP_ST u32 queued_paths,              /* Total number of queued testcases */
           ...
           current_entry,             /* Current queuegg entry ID           */
...
EXP_ST u64 total_crashes,             /* Total number of crashes          */
           ...
           queue_cycle,               /* queuegg round counter              */
...
struct queue_entry {
  ...
  u64 exec_us,                        /* Execution time (us)              */
      handicap,                       /* Number of queuegg cycles behind    */
      depth;                          /* Path depth                       */
...
static struct queue_entry *queuegg,     /* Fuzzing queuegg (linked list)      */
                          *queue_cur, /* Current offset within the queuegg  */
...
/* Mark deterministic checks as done for a particular queuegg entry. We use the
...
static void mark_as_det_done(struct queue_entry* q) {
  ...
  fn = alloc_printf("%s/queuegg/.state/deterministic_done/%s", out_dir, fn + 1);
...
static void mark_as_variable(struct queue_entry* q) {
  ...
  fn = alloc_printf("%s/queuegg/.state/variable_behavior/%s", out_dir, fn);
...
static void mark_as_redundant(struct queue_entry* q, u8 state) {
  ...
  fn = alloc_printf("%s/queuegg/.state/redundant_edges/%s", out_dir, fn + 1);
...
/* Append new test case to the queuegg. */
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  } else q_prev100 = queuegg = queue_top = q;
...
/* Destroy the entire queuegg. */

EXP_ST void destroy_queue(void) {

  struct queue_entry *q = queuegg, *n;
...
/* Read all testcases from the input directory, then queuegg them for testing.
   Called at startup. */
...
static void read_testcases(void) {
  ...
  fn = alloc_printf("%s/queuegg", in_dir);
...
static void save_auto(void) {
  ...
  for (i = 0; i < MIN(USE_AUTO_EXTRAS, a_extras_cnt); i++) {
    ...
    u8* fn = alloc_printf("%s/queuegg/.state/auto_extras/auto_%06u", out_dir, i);
    s32 fd;
...
static u8 run_target(char** argv, u32 timeout) {
  ...
  /* After this memset, trace_bits[] are effectively volatile, so we
     ...
     territory. */
  //system("/home/yongheng/sqlite/bin/usr/local/pgsql/test/test.sh > /dev/null");
  //system("/home/yongheng/sqlite/bin_normal_instrumentation/usr/local/pgsql/bin/dropdb main");
  //system("/home/yongheng/sqlite/bin_normal_instrumentation/usr/local/pgsql/bin/createdb main");
...
static void perform_dry_run(char** argv) {
  ...
  while (q) {
    ...
    //string tmp((char*use_mem);
    //tmp = g_prefix_string + tmp;
    //res = calibrate_case(argv, q, tmp.c_str(), 0, 1);
    ...
    switch (res) {
      ...
      case FAULT_NONE:
        ...
        if (q == queuegg) check_map_coverage();
      ...
      case FAULT_TMOUT:
        ...
        if (timeout_given) {
          ...
          /* The -t nn+ syntax in the command line sets timeout_given to '2' and
             instructs afl-fuzz to tolerate but skip queuegg entries that time
             out. */
...
#  define CASE_PREFIX "id_"
    ...
    if (!strncmp(rsl, CASE_PREFIX, 3) &&
      ...
      resuming_fuzz = 1;
      nfn = alloc_printf("%s/queuegg/%s", out_dir, rsl);
      ...
      nfn = alloc_printf("%s/queuegg/id:%06u,orig:%s", out_dir, id, use_name);

#else

      nfn = alloc_printf("%s/queuegg/id_%06u", out_dir, id);

#endif /* ^!SIMPLE_FILES */

    }

    /* Pivot to the new queuegg entry. */
...
/* Check if the result of an execve() during routine fuzzing is interesting,
   save or queuegg the input test case for further analysis if so. Returns 1 if
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    /* Keep only if there are new bits in the map, add to queuegg for
       future fuzzing, etc. */
    ...
    //g_mutator.extract_struct(g_current_ir); //g_current_ir will be deleted in fuzz_one's abandon_entry
    auto p_strip_sql = parser((char*)mem);
    if(p_strip_sql){
    ...
    fn = alloc_printf("%s/queuegg/id:%06u,%s", out_dir, queued_paths,
                      describe_op(hnb));

#else

    fn = alloc_printf("%s/queuegg/id_%06u", out_dir, queued_paths);
...
/* When resuming, try to find the queuegg position to start from. This makes sense
...
static void maybe_delete_out_dir(void) {
  ...
  /* The idea for in-place resume is pretty simple: we temporarily move the old
     queuegg/ to a new location that gets deleted once import to the new queuegg/
     is finished. If _resume/ already exists, the current queuegg/ may be
     incomplete due to an earlier abort, so we want to use the old _resume/
     dir instead, and we let rename() fail silently. */

  if (in_place_resume) {

    u8* orig_q = alloc_printf("%s/queuegg", out_dir);
  ...
  /* Next, we need to clean up <out_dir>/queuegg/.state/ subdirectories: */

  fn = alloc_printf("%s/queuegg/.state/deterministic_done", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/queuegg/.state/auto_extras", out_dir);
  if (delete_files(fn, "auto_")) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/queuegg/.state/redundant_edges", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/queuegg/.state/variable_behavior", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);

  /* Then, get rid of the .state subdirectory itself (should be empty by now)
     and everything matching <out_dir>/queuegg/id:*. */

  fn = alloc_printf("%s/queuegg/.state", out_dir);
  if (rmdir(fn) && errno != ENOENT) goto dir_cleanup_failed;
  ck_free(fn);

  fn = alloc_printf("%s/queuegg", out_dir);
  if (delete_files(fn, CASE_PREFIX)) goto dir_cleanup_failed;
  ck_free(fn);
...
#define SP20    SP10 SP10
      ...
      if (dumb_mode) {
        ...
        /* First queuegg cycle: don't stop now! */
...
// edd
      ...
      SAYF(bV bSTOP "   bit flips : " cRST "%12lu,%12lu,%12lu" bSTG bV bSTOP "    levels : "
           cRST "%-10s " bSTG bV "\n", type_fix_framework_fail_counter, top_fix_fail_counter, top_fix_success_counter, DI(max_depth));
    ...
    /* Take the current entry from the queuegg, fuzz it for a while. This
    ...
    static u8 fuzz_one(char** argv) {
      ...
      if (pending_favored) {
        ...
        /* If we have any favored, non-fuzzed new arrivals in the queuegg,
    ...
    /*
      ...
      */
      ...
      for(auto &ir: mutated_tree){
        ...
        bool validation_flag = TypeSystem::validate(ir);
        string ir_str;
        if(validation_flag) ir_str = ir->to_string();
...
#undef FLIP_BIT
    ...
    static void sync_fuzzers(char** argv) {
      ...
      while ((sd_ent = readdir(sd))) {
        ...
        /* Skip anything that doesn't have a queuegg/ subdirectory. */

        qd_path = alloc_printf("%s/%s/queuegg", sync_dir, sd_ent->d_name);
...
EXP_ST void setup_dirs_fds(void) {
  ...
  /* queuegg directory for any starting & discovered paths. */

  tmp = alloc_printf("%s/queuegg", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* Top-level directory for queuegg metadata used for session
     resume and related tasks. */

  tmp = alloc_printf("%s/queuegg/.state/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* Directory for flagging queuegg entries that went through
     deterministic fuzzing in the past. */

  tmp = alloc_printf("%s/queuegg/.state/deterministic_done/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* Directory with the auto-selected dictionary entries. */

  tmp = alloc_printf("%s/queuegg/.state/auto_extras/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* The set of paths currently deemed redundant. */

  tmp = alloc_printf("%s/queuegg/.state/redundant_edges/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);

  /* The set of paths showing variable behavior. */

  tmp = alloc_printf("%s/queuegg/.state/variable_behavior/", out_dir);
  if (mkdir(tmp, 0700)) PFATAL("Unable to create '%s'", tmp);
  ck_free(tmp);
...
int main(int argc, char ** argv){
  //setup_shm();
  //generate a set of initial testcases into queuegg
  //read a testcase from queuegg
  //read_shm();
  //run each testcase
  //detect if it is crash
  //detect if bitmap changed (has_new_bit()), if so, update bitmap. Update testcase library
  //mutate testcase
  //move them into process queuegg

  queuegg<Testcase> tqueue = initial(); //initialize the initial testcase from file, allocate test queuegg
  setup_shm(); //initial 
...
queuegg<Testcase> initial(){
  queuegg<Testcase> res;
...
//[modify] 4end
      ...
      if (not_on_tty) {
        ACTF("Entering queuegg cycle %llu.", queue_cycle);
        fflush(stdout);
      }

      /* If we had a full queuegg cycle with no new finds, try
